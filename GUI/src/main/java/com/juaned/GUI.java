...
public class GUI extends JFrame {
    ...
    public static void centerWindow(Window frame) { /// [DONE]
        /*
         * method is to properly start the program that is most convenient to the situation.
         * if it is started with no connected monitors, it will start it and position it in a convenient side of the screen.
         * if it is started with three monitors, it means I am editing code in my home studio,
         * so it will position the program in the second monitor screen.
         */
        ...
        // if there are three monitors, the GUI will start in second monitor. [For coding while I am home]
        if (allScreens.length > 1) {
            myScreen = allScreens[1];
            x = frame.getWidth();
            y = frame.getHeight();
            frame.setLocation(myScreen.getDefaultConfiguration().getBounds().width * 3 / 2 - x, myScreen.getDefaultConfiguration().getBounds().height * 4 / 5 - y);
        }
        // if I am not connected to any other monitors, the GUI will start on right side of default monitor. [Because the left side of my computer is broken]
        else {
    ...
    public static void makeProcessingFullScreen() { /// [DONE]
        ...
        GraphicsEnvironment env = GraphicsEnvironment.getLocalGraphicsEnvironment();         /// returns the local Graphics Environment
        GraphicsDevice[] allScreens = env.getScreenDevices();
        ...
        // else, display notice that there needs to be a second monitor to go fullscreen.
        else {
    ...
    public static void makeProcessingNormalScreen() { /// [DONE]
        /* 
         * sets the window size of the Processing window 
         * to the default size of 1080 x 742.
         */
        ...
        processingFrame.dispose();                                  /// causes the JFrame window to be destroyed and cleaned up by the operating system.
